...
def test_preflight_fails_when_required_env_missing(monkeypatch):
    ...
    result = preflight.run_preflight(logger)
...
def test_preflight_fails_when_prompt_alignment_enabled_without_instructions(monkeypatch):
    ...
    # Stub out deepeval and mongo so only the PA check fires
    monkeypatch.setattr(importlib.util, "find_spec", lambda name: object())
    monkeypatch.setattr(preflight, "MongoClient", _NoopMongoClient)
...
class _NoopAdminDB:
    def command(self, cmd: str) -> None:
        pass  # ping succeeds silently


class _NoopMongoClient:
    def __init__(self, *args, **kwargs):
        self.admin = _NoopAdminDB()
